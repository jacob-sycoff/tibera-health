...
export async function conversationV2(args: {
  text: string;
  ...
  mode?: "chat" | "conversation" | "conversation_v2";
}): Promise<
  | { success: true; data: AssistantV2Response; meta?: { sessionId?: string; turnId?: string } }
  | { success: false; error: string }
> {
  const nowIso = new Date().toISOString();
  const today = new Date().toISOString().slice(0, 10);

  const response = await fetch("/api/assistant/conversation-v2", {
    method: "POST",
  ...
  const outParsed = AssistantV2ResponseSchema.safeParse(parsed.data.data);
